...
class Task extends Component {
  ...
  render() {
    ...
    if (this.state.edit) {
      title = (
        <form onSubmit={this.updateTask.bind(this)} className="edit-task">
          <input type="text" name="title" value={this.state.title} onChange={this.setTitle.bind(this)} className="edit-task" />
        </form>
      );
    }
    ...
    return (
      <div className="row task">
        ...
        <div className="large-3 column text-right">
          <button className="task-button button supertiny primary" type="button" onClick={() => this.setState({edit: !this.state.edit, title: this.props.task.title})}>
          ...
          <button className="task-button button supertiny alert" type="button" onClick={() => this.props.removeTask(this.props.list, task)}>
            <svg version="1.1" baseProfile="basic"
               xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="12px" height="12px"
               viewBox="0 0 128 128" xmlSpace="preserve">
